/* ============================================
   ...
   Vendor libraries + theme scripts compiled for
   Youth Logistic (jQuery Migrate, WP hooks/i18n,
   Contact Form 7, Bootstrap, Owl Carousel,
   Fancybox, AOS, and custom UI helpers)
   ============================================ */
...
/* ============================================
   WORDPRESS HOOKS RUNTIME
   Auto-generated build of @wordpress/hooks API
   ============================================ */
...
/* ============================================
   WORDPRESS I18N & FORMAT UTILITIES
   Auto-generated translation helpers used by
   Contact Form 7 and other WP scripts
   ============================================ */
/*! This file is auto-generated */
( () => {
    var t = {
...
/* ============================================
   CONTACT FORM 7 FRONT-END BUNDLE
   Handles validation, submission, DOM updates
   ============================================ */
var wpcf7 = {
    "api": {
        "root": "https:\/\/yuthlogistics.com.au\/wp-json\/",
...
/* ============================================
   BOOTSTRAP 3.3.7 JAVASCRIPT COMPONENTS
   Vanilla vendor bundle direct from Bootstrap
   ============================================ */
/*!
...
!function(a, b, c, d) {
    ...
    ,
/* ============================================
   OWL CAROUSEL 2 CORE + PLUGINS
   Provides the sliders used throughout the theme
   ============================================ */
...
//
/* ============================================
   FANCYBOX 3 LIGHTBOX + EXTENSIONS
   Core gallery plus thumbs/share/hash/wheel add-ons
   ============================================ */
...
/* ============================================
   ...
   Handles scroll-triggered animation observers
   ============================================ */
;!function(e, t) {
...
/* ============================================
   THEME UI HELPERS & INTERACTIONS
   Youth Logistic custom scripts (menus, sliders,
   Fancybox helpers, placeholder tweaks, etc.)
   ============================================ */
"use strict";
(function($) {
            ...
            if (dropdownMenuLink.length) {
                ...
                dropdownMenuLink.on('mouseleave', function() {
                    ...
                    if (parentLI.length) {
                        ...
                        setTimeout(function() {
                            if (!parentLI.hasClass('mouse-on-sibling')) {
                                parentLI.removeClass('open')
                            }
                        }, 250)
                    }
    ...
    function triggerSideMenuDropdown() {
        // Handle dropdown parent links in mobile menu
        $('.sidebar-nav .menu-item-has-children > a, .sidebar-nav .dropdown > a').on('click', function(e) {
            let $link = $(this);
            let $parent = $link.parent('li');
            let $dropdown = $parent.find('.dropdown-menu');
            
            // If this is a dropdown parent, prevent navigation and toggle dropdown
            if ($parent.hasClass('menu-item-has-children') || $parent.hasClass('dropdown')) {
                e.preventDefault();
                e.stopPropagation();
                
                // Toggle the dropdown
                $parent.toggleClass('bagels-open');
                $parent.toggleClass('open');
                $parent.removeClass('bagels-close');
                
                // Close other open dropdowns
                $parent.siblings('.bagels-open, .open').removeClass('bagels-open open').addClass('bagels-close');
            }
        });
        
        // Handle regular menu item links (non-dropdown)
        $('.sidebar-nav .menu-item:not(.menu-item-has-children):not(.dropdown) > a').on('click', function(e) {
...
(function($) {
    ...
    function getCommonServicesSliderSettings() {
        return {
            loop: !0,
            dots: !0,
            nav: !0,
    ...
    function triggerMainMenuButtonPopup() {
        let self = this;
        self.menuButon = $('#site-header .vc-theme-button a');
        if (self.menuButon.length) {
            self.menuButon.on('click', function(e) {
                e.preventDefault();
                let popupTrigger = $(".nav-popup-trigger");
                if (popupTrigger.length) {
                    popupTrigger.fancybox().trigger('click')
                }
            })
        }
    }
    function setSameBoxesHeight(boxes) {
        let tallestHeight = 0;
        if (boxes.length) {
            $.each(boxes, function(key, value) {
                let thisBoxObj = $(value);
                let thisBoxHeight = thisBoxObj.outerHeight();
                if (thisBoxHeight > tallestHeight) {
                    tallestHeight = thisBoxHeight
                }
            });
            boxes.css({
    ...
    function initHomeServicesSliderActions() {
        let self = this;
        self.setSameHeightSlides = function() {
            setSameBoxesHeight($('.pg-home .ph-services .ph-svs-s-single'))
        }
        self.initHomeServiceSlider = function() {
            let homeServicesSlider = $('.pg-home .ph-services .ph-svs-s-1.owl-carousel');
            if ($(window).width() < 768) {
                if (homeServicesSlider.length) {
                    homeServicesSlider.removeClass('owl-carousel')
                }
            } else {
                if (homeServicesSlider.length) {
                    homeServicesSlider.owlCarousel(getCommonServicesSliderSettings());
                    self.setSameHeightSlides();
                    homeServicesSlider.on('changed.owl.carousel', function(e) {
                        self.setSameHeightSlides()
                    })
                }
            }
        ...
        ;
        self.initHomeServiceSlider()
    }
    function initSinServiceRelServicesSliderActions() {
        let self = this;
        self.setSameHeightSlides = function() {
            setSameBoxesHeight($('.pc-sin-service .pc-ss-ms-related-services .pc-as-s-s-1'))
        }
        ;
        self.initSlider = function() {
            let slider = $('.pc-sin-service .pc-ss-ms-related-services .pc-ss-rp-list');
            if (slider.length) {
                slider.owlCarousel(getCommonServicesSliderSettings());
                if ($(window).width() > 767) {
                    self.setSameHeightSlides()
                }
            }
    ...
    function initAboutUsServicesSliderActions() {
        let self = this;
        self.setSameHeightSlides = function() {
            setSameBoxesHeight($('.pg-about-us .pg-au-related-services .ph-svs-s-single'))
        }
        ;
        self.initSlider = function() {
            let slider = $('.pg-about-us .pg-au-related-services .pg-au-rs-list.owl-carousel');
            if (slider.length) {
                slider.owlCarousel(getCommonServicesSliderSettings());
                if ($(window).width() > 767) {
                    self.setSameHeightSlides()
    ...
    function initLibs() {
        let self = this;
        const cmnSliderSettings = getCommonOwlSettings();
        self.brandsSlider = $('.trusted-brands .tb-1.owl-carousel');
        self.blogsSlider = $('.ph-blogs .ph-bl-bb-1.owl-carousel');
        self.tmnlsSlider = $('.ph-testimonials .ph-tmns-right.owl-carousel');
        self.projsSlider = $('.pg-home .ph-prj-projects, .pc-sin-projects .pc-ss-rp-list');
        self.brandsSettings = Object.assign({}, cmnSliderSettings);
        self.blogsSettings = Object.assign({}, cmnSliderSettings);
        self.tmnlsSettings = Object.assign({}, cmnSliderSettings);
        self.projsSliderSettings = Object.assign({}, cmnSliderSettings);
        if (self.brandsSlider.length) {
            self.brandsSettings.dots = !1;
            self.brandsSettings.nav = !1;
            self.brandsSettings.items = 5;
            self.brandsSettings.margin = 10;
            self.brandsSettings.responsive = {
                0: {
                    items: 2,
                    margin: 5,
                },
            ...
            self.brandsSlider.owlCarousel(self.brandsSettings)
        }
        if (self.blogsSlider.length) {
            self.blogsSettings.nav = !1;
            self.blogsSettings.items = 3;
            self.blogsSettings.margin = 30;
            self.blogsSettings.responsive = {
                0: {
                    items: 1,
                    margin: 20,
                },
            ...
            self.blogsSlider.owlCarousel(self.blogsSettings)
        }
        if (self.tmnlsSlider.length) {
            self.tmnlsSettings.nav = !1;
            self.tmnlsSettings.items = 1;
            self.tmnlsSettings.margin = 30;
            self.tmnlsSlider.owlCarousel(self.tmnlsSettings)
        }
        if (self.projsSlider.length) {
            self.projsSliderSettings.nav = !1;
            self.projsSliderSettings.items = 1;
            self.projsSliderSettings.margin = 20
        }
        self.destroyOwlCarousel = function(carousel) {
            if (carousel.length) {
                carousel.trigger('destroy.owl.carousel');
                carousel.removeClass('owl-carousel')
            }
        }
        self.initializeOwlCarousel = function(carousel, settings) {
            if (carousel.length) {
                carousel.addClass('owl-carousel');
                carousel.owlCarousel(settings)
            }
        }
        self.toggleOwlCarouselStatus = function() {
            if ($(window).width() < 768) {
                self.initializeOwlCarousel(self.projsSlider, self.projsSliderSettings)
            } else {
                self.destroyOwlCarousel(self.projsSlider)
            }
        }
        self.toggleOwlCarouselStatus();
        $(window).on('resize', function() {
            self.toggleOwlCarouselStatus()
        });
        self.initGalleryGrid = function() {
            let galleryGrid1 = $('.gallery-grid-style-1');
            if (galleryGrid1.length) {
                let sliderTrigger = galleryGrid1.find('[data-trigger-slider]');
                if (sliderTrigger.length) {
                    sliderTrigger.on('click', function() {
                        let singleElem = galleryGrid1.find('.ggs1-single');
                        if (singleElem.length) {
                            let vidElem = singleElem.first();
                            if (vidElem.length) {
                                let trigger = vidElem.find('.ggs1-s-1');
                                if (trigger.length) {
    ...
    function replacePlaceholders() {
        ...
        if (selectField) {
            ...
            if (firstOption) {
                if (!firstOption.val()) {
                    selectField.css('color', '#9b9999');
                    options.css('color', "#262626");
                    firstOption.css('color', '#9b9999');
                    selectField.on('change', function() {
                        let color = !this.value ? '#9b9999' : '#262626';
